...
class Parser(api.Parser):
    def __init__(self, name: str, log: logger.Logger, provider_: SubProvider):
        ...
        self.link: str = 'https://www.capsuletoronto.com/collections/new-arrivals/footwear?page=1'
        self.interval: int = 1
        ...
        if mode == 0:
            ...
            for element in etree.HTML(self.provider.get(self.link,
                ...
                if 'yeezy' in element.get('href') or 'air' in element.get('href') or 'dunk' in element.get('href') \
                    ...
                    links.append([api.Target('https://www.capsuletoronto.com' + element.get('href'), self.name, 0),
                                  'https://www.capsuletoronto.com' + element.get('href')])
                counter += 1
            if len(links) == 0:
                return result
            for link in links:
                try:
                    if HashStorage.check_target(link[0].hash()):
                        ...
                        sizes = [api.Size(str(size_data.current_value['public_title'].split(' ')[-1]) + ' US',
                                          'https://www.capsuletoronto.com/cart/' + str(
                        ...
                        result.append(IRelease(
                            ...
                            [
                                FooterItem('StockX', 'https://stockx.com/search/sneakers?s=' +
                                           name.replace(' ', '%20')),
                                FooterItem('Cart', 'https://www.capsuletoronto.com/cart'),
