...
class Parser(api.Parser):
    def __init__(self, name: str, log: Logger):
        ...
        self.catalog: str = 'https://www.capsuletoronto.com/collections/new-arrivals/footwear?page=1'
        self.interval: int = 1
    ...
    def targets(self) -> List[TargetType]:
        ...
        for element in etree.HTML(get(self.catalog,
                ...
                .xpath('//ul[@class="collection-products"]/li/a'):
            if counter == 5:
                break
        ...
        return [
            api.TInterval(element.split('/')[-1],
                          self.name, 'https://www.capsuletoronto.com' + element, self.interval)
        ...
        try:
            if isinstance(target, api.TInterval):
                ...
                available_sizes = list(size.get('data-value')
                                       for size in content.xpath('//div[@class="swatch clearfix"]/div')
                ...
                for size in Path.parse_str('$.product.variants.*').match(
                    ...
                    if size.current_value['public_title'] in available_sizes:
                        sizes.append(
                            (
                                str(size.current_value['public_title']) + ' US',
                                'https://www.capsuletoronto.com/cart/' + str(size.current_value['id']) + ':1'
                            )
                        )
            else:
        ...
        if len(available_sizes) > 0:
            ...
            return api.SSuccess(
                ...
                api.Result(
                    ...
                    {'Site': 'Capsule Toronto'},
                    tuple(sizes),
                    (
                        ('StockX', 'https://stockx.com/search/sneakers?s=' + name.replace(' ', '%20')),
                        ('Cart', 'https://www.capsuletoronto.com/cart'),
