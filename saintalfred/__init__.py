...
class Parser(api.Parser):
    def __init__(self, name: str, log: logger.Logger, provider_: SubProvider):
        ...
        self.link: str = 'https://www.saintalfred.com/collections/brands?sort_by=created-descending'
        self.interval: int = 1
                ...
                try:
                    if HashStorage.check_target(link[0].hash()):
                        ...
                        sizes = [api.Size(str(size_data.current_value['public_title']).split(' ')[0] + ' US',
                                          'https://www.saintalfred.com/cart/' + str(
                                              size_data.current_value['id']) + ':1')
                                 for size_data in sizes_data
                                 if page_content.xpath(f'//option[@value="{size_data.current_value["id"]}"]')[0].get(
                                'disabled') is None]
