...
class Parser(api.Parser):
    ...
    def execute(
        ...
        if mode == 0:
            result.append(TInterval('catalog_1', self.name, ['yeezy',
                                                             'https://www.farfetch.com/ru/plpslice/listing-api/products-facets?view=90&scale=282&gender=Women|Men&pagetype=Shopping&designer=4968477&c-category=137174&page=2&pagetype=Search&pricetype=FullPrice'],
                                    5))
            result.append(TInterval('catalog_2', self.name, ['yeezy',
                                                             'https://www.farfetch.com/ru/plpslice/listing-api/products-facets?view=90&scale=282&gender=Women|Men&pagetype=Shopping&designer=4968477&c-category=137174&pagetype=Search&pricetype=FullPrice'],
                                    5))
            result.append(TInterval('catalog_3', self.name, ['nike_x_off-white',
                                                             'https://www.farfetch.com/ru/plpslice/listing-api/products-facets?view=90&scale=282&gender=Women|Men&pagetype=Shopping&pricetype=FullPrice&designer=12264703&c-category=135968&pagetype=Search&pricetype=FullPrice'],
                                    5))
        ...
        if mode == 1:
            try:
                if 'catalog' in content.name:
                    ...
                    for item in json['listingItems']['items']:
                        ...
                        if availability:
                            ...
                            if not 'powerphase' in name.lower() or not '380' in name.lower():
                                result.append(
                                    api.TScheduled(
                                        link,
                                        self.name,
                    ...
                    try:
                        ...
                        for size, size_data in json_sizes.items():
                            if size_data['available']:
                                ...
                                if min_price > content.data[7]:
                                    if float(size_data["formattedFinalPrice"].replace('₽', '').replace('\xa0',
                                                                                                       '')) == min_price:
                        ...
                        result.append(
                            IRelease(
                                content.name + f'#sprices={sizes.hash().hex()}',
                                channel,
                                content.data[0],
                                content.data[1],
                                '',
                ...
                result.append(api.MAlert(f'Script is down {content.name}', self.name))
                result.append(content)
                return result
