...
class Parser(api.Parser):
    def __init__(self, name: str, log: logger.Logger, provider_: SubProvider):
        ...
        self.link: str = 'https://www.notre-shop.com/products.json?collection=new-arrivals,footwear&limit=50'
        self.interval: int = 1
            ...
            try:
                ...
                for element in Path.parse_str('$.products.*').match(loads(products)):
                    if 'yeezy' in element.current_value['handle'] or 'air' in element.current_value['handle'] \
                        ...
                        target = api.Target('https://www.notre-shop.com/collections/footwear/products/' + element.
                                            current_value['handle'], self.name, 0)
                        if HashStorage.check_target(target.hash()):
                            sizes = [api.Size(str(size.current_value['option1']) + ' US',
                                              f'https://www.notre-shop.com/cart/{size.current_value["id"]}:1')
